...
export default async function handler(req, res) {
  ...
  const checkRes = await fetch(
    `${SUPABASE_URL}/rest/v1/bull_pen_strict_hof?ticker=in.(${tickerList})&detected_at=gte.${cutoff}&select=ticker`,
    {
  ...
  if (!checkRes.ok) {
    console.error('[STRICT record] dedup check failed:', checkRes.status);
  ...
  const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/bull_pen_strict_hof`, {
    method: 'POST',
    headers: {
      apikey:         SUPABASE_SVC,
  ...
  if (!insertRes.ok) {
    console.error('[STRICT record] insert error:', await insertRes.text());
